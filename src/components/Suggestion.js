...
export default function Suggestion({ $target, initialState, onSelect }) {
    ...
    this.renderMatchedItems = (keyword, item) => {
    ...
    // Suggestion__item--matched  선택되고 하위 검색 같은 느낌
    // Suggestion__item--selected 검색 되었을 때,
    this.render = () => {
        ...
        if(items.length > 0 ) {
            ...
            <ul>
                ${items?.map((item, index) => `
                    <li data-index=${index} class="${index === selectedIndex ? 'Suggestion__item--selected' : ''}">${this.renderMatchedItems(keyword, item)}</li>
                `)
                .join('')}
            </ul>
        `
